...
class RecipeViewSet(viewsets.ModelViewSet):
    ...
    def download_shopping_cart(self, request):
        ...
        ).values_list("ingredients__name", "ingredients__measurement_unit", "amount")
...
class FollowViewSet(viewsets.ModelViewSet):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        user_id = request.user.id
        Follow.objects.filter(user__id=user_id, author__id=author_id).delete()
        return Response(status=status.HTTP_204_NO_CONTENT)
...
class ShoppingViewSet(viewsets.ModelViewSet):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        recipe_id = self.kwargs["id"]
        user_id = request.user.id
        ShoppingList.objects.filter(user__id=user_id, recipes__id=recipe_id).delete()
        return Response(status=status.HTTP_204_NO_CONTENT)
